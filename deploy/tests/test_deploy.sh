...
test_deploy_retries_smoke_check_until_backend_is_ready() {
  local temp_dir
  temp_dir="$(mktemp -d)"
  trap 'rm -rf "$temp_dir"' RETURN
  ...
  local bin_dir="$temp_dir/bin"
  local command_log="$temp_dir/commands.log"
  local curl_state="$temp_dir/curl-attempts"
...
EOF
  ...
  chmod +x "$bin_dir/docker" "$bin_dir/sleep" "$bin_dir/curl"

  PATH="$bin_dir:$PATH" PROJECT_ROOT="$project_dir" bash "$SCRIPT_PATH"

  assert_contains "$command_log" "sleep 2"
  if [[ "$(cat "$curl_state")" != "3" ]]; then
    fail "deploy.sh should retry smoke check until curl succeeds"
  fi
}
